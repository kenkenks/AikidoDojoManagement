/* Generated: node tools/build-time-travel.mjs */
var DojoTimeTravel=(function(){const modules={"./Flow.js":function(module,exports,require){
'use strict';
...
"./MemberObject.js":function(module,exports,require){
'use strict';
...
'use strict';
const { normalizeMemberId, memberObject } = require('./MemberObject.js');

// 保存先・項目対応の一覧。左が返却項目、右が保存先の項目名。
// コレクションを増やすときは、この一覧に並べる。
const definitions = {
  setting: {
    writable: ['key', 'value'],
    sources: {
  ...
  member: {
    normalizeId: normalizeMemberId,
    validate: memberObject,
    sources: {
      firestore: {
        collection: 'members', keyField: 'member_id',
        fields: { member_id: 'member_id', name: 'name', status: 'status' }
      },
      sheets: {
        sheet: '01_会員マスタ', keyField: 'member_id',
        fields: { member_id: 'member_id', name: '氏名', status: '状態' },
        transforms: { member_id: value => String(value ?? '').trim() }
      }
    }
  }
};
// 出席移植で使う原簿。業務項目名はGASと共通のまま保持する。
...
"./TimeApplication.js":function(module,exports,require){
'use strict';
...
}};const cache={};function require(name){if(!Object.prototype.hasOwnProperty.call(modules,name))throw new Error(name);if(!cache[name]){const m={exports:{}};cache[name]=m;modules[name](m,m.exports,require);}return cache[name].exports;}return require("./TimeApplication.js");})();
